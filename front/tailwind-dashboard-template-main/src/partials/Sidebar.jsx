...
function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  useEffect(() => {
    JSON.parse(localStorage.getItem('userInfo')).roles?.map(role => role === 'ROLE_ADMIN' ? setIsAdmin(true) : setIsAdmin(false))
  }, [])
                  ...
                  {(handleClick, open) => {
                    return (
                      <React.Fragment>
                        <a
                          href="#0"
                          className={`block text-slate-200 hover:text-white truncate transition duration-150 ${(pathname === '/main' || pathname.includes('dashboard')) && 'hover:text-slate-200'
                            }`}
                          onClick={(e) => {
                            e.preventDefault();
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              <svg className="shrink-0 h-6 w-6" viewBox="0 0 24 24">
                                <path
                                  className={`fill-current text-slate-400 ${(pathname === '/main' || pathname.includes('dashboard')) && '!text-indigo-500'
                                    }`}
                    ...
                    {(handleClick, open) => {
                      return (
                        <React.Fragment>
                          <a
                            href="#0"
                            className={`block text-slate-200 hover:text-white truncate transition duration-150 ${pathname.includes('admin') && 'hover:text-slate-200'
                              }`}
                            onClick={(e) => {
                              e.preventDefault();
                              sidebarExpanded ? handleClick() : setSidebarExpanded(true);
                  ...
                  {(handleClick, open) => {
                    return (
                      <React.Fragment>
                        <a
                          href="#0"
                          className={`block text-slate-200 hover:text-white truncate transition duration-150 ${pathname.includes('faq') && 'hover:text-slate-200'
                            }`}
                          onClick={(e) => {
                            e.preventDefault();
                            sidebarExpanded ? handleClick() : setSidebarExpanded(true);
